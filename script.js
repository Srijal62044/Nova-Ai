...
/**
 * Build fetch headers for the Gemini API.
 * - Legacy keys (AIza...) go in the URL as ?key=
 * - New provisioned keys (AQ...) go as Authorization: Bearer
 */
...
function geminiUrl(model, apiVersion = 'v1beta') {
  const base = `https://generativelanguage.googleapis.com/${apiVersion}/models/${model}:generateContent`;
  if (GEMINI_API_KEY.startsWith('AQ')) return base;
  return `${base}?key=${GEMINI_API_KEY}`;
}
