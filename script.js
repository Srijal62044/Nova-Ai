...
// Custom marked renderer — wraps code blocks with our UI
...
async function callGemini(contents) {
  const res = await fetch(GEMINI_CHAT_URL, {
    ...
    body: JSON.stringify({
      ...
      systemInstruction: {
        parts: [{
          text: `You are Nova AI, a highly intelligent, helpful, and articulate AI assistant powered by Google Gemini. You provide accurate, thoughtful, and well-structured responses. You support markdown formatting in your responses including code blocks, lists, tables, and emphasis. You are friendly but professional.`
        }]
      }
    }),
  });
...
async function handleImageGeneration(chatId, prompt) {
  ...
  try {
    ...
    removeGeneratingIndicator(genEl);
    showErrorMessage('Image generation failed: ' + (err.message || 'Unknown error'));
  }
...
function showErrorMessage(text) {
  ...
  div.className = 'message ai';
  div.innerHTML = `
    <div class="msg-avatar" style="background:#ef444420;color:#ef4444">
