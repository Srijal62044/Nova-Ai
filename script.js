...
// Chat models tried in order — if the first is quota-exceeded, we fall back automatically
...
async function callGemini(contents) {
  ...
  for (const model of GEMINI_CHAT_MODELS) {
    ...
    // Only fall back to the next model on quota / rate-limit / service errors
    if (status === 429 || status === 503) {
      lastError = parseGeminiError(status, rawMsg);
      continue; // try next model
    }

    // For all other errors (400, 401, 403, etc.) fail immediately — retrying won't help
...
async function callGeminiImage(prompt) {
  ...
  if (!res.ok) {
    ...
    throw new Error(parseGeminiError(res.status, err?.error?.message || ''));
  }
