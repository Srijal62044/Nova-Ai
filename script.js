/* ============================================================
   NOVA AI — script.js
   Full application logic: OpenRouter chat, HuggingFace image
   generation, chat management, voice, settings, and more.
   ============================================================ */
...
const OPENROUTER_MODELS_URL = 'https://openrouter.ai/api/v1/auth/key';
...
const NOVA_SYSTEM_PROMPT = `You are Nova AI, a highly intelligent, helpful, and articulate AI assistant. You provide accurate, thoughtful, and well-structured responses. You support markdown formatting including code blocks, lists, tables, and emphasis. You are friendly but professional.`;

/* ── Key helpers — read from localStorage, never hardcoded ── */
function getOpenRouterKey() { return localStorage.getItem('nova_openrouter_key') || ''; }
function getHFToken()       { return localStorage.getItem('nova_hf_token') || ''; }

/** Human-readable API error messages */
function parseApiError(status, rawMessage) {
  if (!rawMessage) rawMessage = '';
  if (!navigator.onLine) return 'You appear to be offline. Check your internet connection.';
  if (status === 401 || status === 403) return 'API key is invalid or expired. Go to Settings and update it.';
  if (status === 429) return 'Rate limit reached. Please wait a moment and try again.';
  if (status === 402) return 'Insufficient credits. Top up your OpenRouter account.';
  if (status === 400) return 'Invalid request — please rephrase your message.';
  if (status === 500 || status === 503) return 'The AI service is temporarily unavailable. Try again in a moment.';
  return rawMessage.slice(0, 160) || `API error ${status}.`;
}
...
async function autoGenerateTitle(chatId, firstMessage) {
  try {
    const res = await callOpenRouter([
      { role: 'user', content: `Summarize this message as a chat title in 4-6 words max, no quotes, no punctuation: "${firstMessage}"` }
    ]);
...
async function sendMessage() {
  ...
  // Call OpenRouter
  try {
    const messages = buildOpenRouterMessages(chat.messages); // includes the new msg
...
/** Build the OpenRouter-compatible conversation history */
...
async function callOpenRouter(messages) {
  ...
  if (!key) throw new Error('No OpenRouter API key set. Go to Settings and enter your key.');
  ...
  try {
    ...
    throw new Error('Unable to reach OpenRouter. Check your internet connection.');
  }

  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(parseApiError(res.status, err?.error?.message || ''));
  }
...
/**
 * Validate an OpenRouter key by hitting the /auth/key endpoint.
 * @returns {Promise<{valid:boolean, label:string}>}
 */
async function validateOpenRouterKey(key) {
  try {
    ...
    if (res.ok) {
      ...
      const label = d?.data?.label || 'Valid key';
      return { valid: true, label };
    }
    return { valid: false, label: parseApiError(res.status, '') };
  } catch {
    return { valid: false, label: 'Network error — check your connection.' };
  }
}

/* ============================================================
   HUGGING FACE — FLUX Image Generation
============================================================ */
/**
 * Generate an image using HuggingFace FLUX.1-schnell.
 * @param {string} prompt
...
/**
 * Validate a Hugging Face token via the whoami endpoint.
 * @returns {Promise<{valid:boolean, label:string}>}
 */
async function validateHFToken(token) {
  try {
    ...
    if (res.ok) {
      const d = await res.json();
      return { valid: true, label: d?.name || 'Valid token' };
    }
    return { valid: false, label: parseApiError(res.status, '') };
  } catch {
    return { valid: false, label: 'Network error — check your connection.' };
...
/* ============================================================
   KEY MANAGEMENT — Settings UI helpers
============================================================ */
/** Save both keys to localStorage */
function saveApiKeys() {
  const orKey = document.getElementById('orKeyInput')?.value.trim() || '';
  const hfKey = document.getElementById('hfTokenInput')?.value.trim() || '';
  if (orKey) localStorage.setItem('nova_openrouter_key', orKey);
  if (hfKey) localStorage.setItem('nova_hf_token', hfKey);
  showToast('Keys saved!', 'success');
  updateKeyStatus();
}

/** Validate and save the OpenRouter key, show result inline */
async function validateAndSaveORKey() {
  const input = document.getElementById('orKeyInput');
  const status = document.getElementById('orKeyStatus');
  const btn = document.getElementById('orValidateBtn');
  const key = input?.value.trim();
  if (!key) { showToast('Enter an OpenRouter key first.', 'error'); return; }

  btn.disabled = true;
  btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Checking…';
  setKeyStatus(status, 'checking', 'Validating…');

  const result = await validateOpenRouterKey(key);
  btn.disabled = false;
  btn.innerHTML = '<i class="fa-solid fa-check"></i> Validate';

  if (result.valid) {
    localStorage.setItem('nova_openrouter_key', key);
    setKeyStatus(status, 'valid', `✓ ${result.label}`);
    showToast('OpenRouter key saved!', 'success');
  } else {
    setKeyStatus(status, 'invalid', `✗ ${result.label}`);
    showToast('Invalid OpenRouter key.', 'error');
  }
}

/** Validate and save the HuggingFace token, show result inline */
async function validateAndSaveHFToken() {
  const input = document.getElementById('hfTokenInput');
  const status = document.getElementById('hfTokenStatus');
  const btn = document.getElementById('hfValidateBtn');
  ...
  if (result.valid) {
    localStorage.setItem('nova_hf_token', token);
    setKeyStatus(status, 'valid', `✓ ${result.label}`);
    showToast('Hugging Face token saved!', 'success');
  } else {
    setKeyStatus(status, 'invalid', `✗ ${result.label}`);
    showToast('Invalid Hugging Face token.', 'error');
  }
}

function setKeyStatus(el, type, text) {
  if (!el) return;
  el.textContent = text;
  el.className = 'key-status key-status--' + type;
}

/** Populate status badges when settings is opened.
 *  Input fields are intentionally left blank so the user
 *  types a fresh key if they want to update — we never
 *  re-populate with the stored value to avoid accidental
 *  submission of a stale masked string.
 */
function updateKeyStatus() {
  const orKey = getOpenRouterKey();
  const hfTok = getHFToken();

  setKeyStatus(document.getElementById('orKeyStatus'),
    orKey ? 'valid' : 'missing',
    orKey ? '✓ Key saved — enter a new one to update' : '⚠ Not set');
  setKeyStatus(document.getElementById('hfTokenStatus'),
    hfTok ? 'valid' : 'missing',
    hfTok ? '✓ Token saved — enter a new one to update' : '⚠ Not set');
}

/** Show only first/last 4 chars of a key for display */
...
/** Clear a key from storage and input */
function clearKey(type) {
  if (type === 'or') {
    localStorage.removeItem('nova_openrouter_key');
    const inp = document.getElementById('orKeyInput');
    if (inp) inp.value = '';
    setKeyStatus(document.getElementById('orKeyStatus'), 'missing', '⚠ Not set');
    showToast('OpenRouter key removed.', 'success');
  } else {
    localStorage.removeItem('nova_hf_token');
    const inp = document.getElementById('hfTokenInput');
    if (inp) inp.value = '';
    setKeyStatus(document.getElementById('hfTokenStatus'), 'missing', '⚠ Not set');
    showToast('Hugging Face token removed.', 'success');
  }
}
...
async function handleImageGeneration(chatId, prompt) {
  ...
  try {
    const dataUrl = await callHuggingFaceImage(prompt);
